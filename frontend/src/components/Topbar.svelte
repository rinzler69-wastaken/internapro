<script>
  ...
  const currentTitle = $derived(
    titles.find((t) => path === t.match || path.startsWith(t.match + "/"))
      ?.label || "Dashboard",
  );
  const role = $derived(auth.user?.role || "intern");
  const settingsHref = $derived(
    role === "intern" || role === "supervisor" || role === "pembimbing"
      ? "/profile/edit"
      : "/settings",
  );
  ...
  function getSubtitle(routePath, userRole, name) {
    ...
    if (routePath.startsWith("/tasks")) {
      return userRole === "intern"
        ? "Pantau tugas dan deadline pribadi."
        : "Kelola penugasan untuk siswa magang.";
    }
    ...
    if (routePath.startsWith("/assessments")) {
      return userRole === "intern"
        ? "Lihat penilaian kinerja anda."
        : "Evaluasi performa siswa magang.";
    }
    if (routePath.startsWith("/reports")) {
      return userRole === "intern"
        ? "Kirim dan kelola laporan kegiatan."
        : "Tinjau laporan periodik siswa.";
    }
...
<div
  class="topbar {collapsed ? 'sidebar-collapsed' : ''}"
...
>
  ...
  <div class="topbar-actions">
    ...
    <div class="dropdown-container">
      ...
      >
        <span
          ><span class="material-symbols-outlined mt-1">notifications</span
          ></span
        >
        {#if unreadCount > 0}
      ...
      {#if notifDropdownOpen}
        <div id="notif-dropdown-menu" class="dropdown-menu notif-dropdown">
                  ...
                  onclick={async () => {
                    ...
                    if (!isRead) {
                      ...
                      try {
                        ...
                        console.log("✅ Server marked read for ID:", id);
                      } catch (err) {
          ...
          <a href="/notifications" class="dropdown-footer"> Lihat Semua </a>
        </div>
      {/if}
    </div>
...
{#if showLogoutModal}
  ...
  >
    ...
    ></div>
    <div
      class="relative bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 flex flex-col items-center text-center overflow-hidden"
    >
      <div
        class="w-12 h-12 rounded-full bg-slate-50 text-slate-500 flex items-center justify-center mb-4"
      >
      ...
      <div class="flex gap-3 w-full">
        <button
          onclick={() => (showLogoutModal = false)}
          class="flex-1 px-4 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-xl hover:bg-slate-50 font-medium text-sm transition-all flex items-center justify-center gap-2 cursor-pointer"
        >
          <span class="material-symbols-outlined text-[18px] cursor-pointer"
            >close</span
          >
          Tidak
        </button>
