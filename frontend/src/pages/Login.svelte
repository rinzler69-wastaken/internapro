<script>
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (msg.toLowerCase().includes('belum disetujui') || msg.toLowerCase().includes('pending')) {
        pendingApproval = true;
...
// Handler Login Google (Backend Redirect Flow)
  ...
  async function handleCancelRegistration() {
    if (!confirm('Apakah Anda yakin ingin membatalkan pendaftaran? Data Anda akan dihapus dan Anda dapat mendaftar ulang.')) return;
    try {
    ...
    <div class="form-body">
      ...
      <div class="form-group relative">
        ...
        <div class="password-wrapper">
            ...
            />
            <button class="toggle-password" onclick={() => showPassword = !showPassword} type="button" aria-label="Toggle password visibility">
            {#if showPassword}
        ...
        <div style="text-align: right; margin-top: 6px;">
             <a href="/forgot-password" use:route class="link-muted small">Lupa password?</a>
        </div>
      </div>
