<script>
  ...
  async function downloadTemplate() {
    ...
    } catch(e) { alert('Gagal download template'); }
  }
...
<div class="page-bg">
  <div class="container animate-fade-in">
    ...
    <div class="header">
        <div>
            <h2 class="title">Pusat Data</h2>
            <p class="subtitle">Kelola eksport dan import data peserta magang dengan mudah.</p>
        </div>
    ...
    <div class="grid-layout animate-slide-up">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="form-group">
                    <label class="label" for="export-intern-status">Status</label>
                    <select class="input-field" id="export-intern-status" bind:value={internStatus}>
                ...
                <button class="btn-outline w-full mt-4" onclick={exportInterns} disabled={isExporting}>
            ...
            <div class="card-body">
                <div class="grid-2">
                    <div class="form-group">
                        ...
                        <input class="input-field" type="date" id="att-start" bind:value={attendanceStart} />
                    </div>
                    <div class="form-group">
                        <label class="label" for="att-end">Sampai</label>
                        <input class="input-field" type="date" id="att-end" bind:value={attendanceEnd} />
                    </div>
                </div>
                <div class="form-group mt-3">
                    <label class="label" for="att-status">Status Kehadiran</label>
                    <select class="input-field" id="att-status" bind:value={attendanceStatus}>
                ...
                <button class="btn-outline w-full mt-4" onclick={exportAttendance} disabled={isExporting}>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
                    Download Excel
                </button>
            </div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="form-group">
                    <label class="label" for="task-intern">Filter Intern</label>
                    <select class="input-field" id="task-intern" bind:value={taskIntern}>
                ...
                <div class="form-group mt-3">
                    <label class="label" for="task-status">Status Tugas</label>
                    <select class="input-field" id="task-status" bind:value={taskStatus}>
                        <option value="">Semua</option>
                        <option value="pending">Pending</option>
                ...
                <button class="btn-outline w-full mt-4" onclick={exportTasks} disabled={isExporting}>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
                    Download Excel
                </button>
            </div>
    ...
    <!-- IMPORT SECTION -->
    <div class="import-section animate-slide-up" style="animation-delay: 0.1s;">
        <div class="card import-card">
            <div class="import-header">
                <div class="header-text">
                    <h3>Import Data Peserta</h3>
                    <p>Tambahkan banyak peserta sekaligus menggunakan file Excel/CSV.</p>
                </div>
                <button class="btn-text" onclick={downloadTemplate}>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 15v4c0 1.1.9 2 2 2h14a2 2 0 0 0 2-2v-4M17 9l-5 5-5-5M12 12.8V2.5"/></svg>
                    Download Template
                </button>
            </div>
            ...
            <div class="import-body">
                <div class="form-group mb-4">
                    <label class="label" for="file-upload">Upload File (.xlsx, .csv)</label>
                    <label class="file-drop" for="file-upload">
                        <div class="drop-content">
                            <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="text-slate-400 mb-2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
                            {#if importFile}
                                <span class="file-name">{importFile.name}</span>
                            {:else}
                                <span class="placeholder">Klik untuk memilih file</span>
                            {/if}
                        </div>
                        <input 
                            id="file-upload" 
                ...
                {#if auth.user?.role === 'admin'}
                    <div class="form-group mb-4">
                        <label class="label" for="import-supervisor">Assign Pembimbing (Opsional)</label>
                        <select class="input-field" id="import-supervisor" bind:value={importSupervisor}>
                ...
                <div class="action-footer">
                    {#if importResult}
                        <div class="result-badge">
                            ...
                            Import Selesai: <b>{importResult.imported}</b> sukses, <b>{importResult.skipped}</b> dilewati.
                        </div>
                    {/if}
                    <button class="btn-primary" onclick={importInterns} disabled={!importFile || isImporting}>
...
<style>
  ...
  .section-title h3 { font-size: 18px; font-weight: 600; color: #1e293b; margin: 0 0 4px 0; }
  .section-title p { font-size: 13px; color: #64748b; margin: 0; }
  ...
  .card-header h4 { font-size: 15px; font-weight: 600; color: #1e293b; margin: 0; }
  ...
  /* GRID LAYOUT */
  .grid-layout { display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 24px; margin-bottom: 40px; }
  ...
  .input-field {
    ...
    font-size: 13px; color: #1e293b; transition: all 0.2s; box-sizing: border-box; background: #fff;
  }
  .input-field:focus { outline: none; border-color: #10b981; box-shadow: 0 0 0 3px rgba(16, 185, 129, 0.1); }

  /* BUTTONS */
  .btn-outline {
    width: 100%; padding: 10px; background: white; border: 1px solid #cbd5e1;
    color: #475569; font-weight: 600; font-size: 13px; border-radius: 8px;
  ...
  .btn-primary {
    ...
    color: white; border: none; font-weight: 600; font-size: 14px; border-radius: 8px;
    cursor: pointer; transition: all 0.2s;
  }
  .btn-primary:hover { box-shadow: 0 4px 12px rgba(16, 185, 129, 0.3); transform: translateY(-1px); }
  .btn-primary:disabled { background: #94a3b8; cursor: not-allowed; box-shadow: none; transform: none; }

  .btn-text {
    background: transparent; border: none; color: #10b981; font-weight: 600; font-size: 13px;
  ...
  .import-card { border: 1px dashed #cbd5e1; }
  .import-header {
    padding: 24px; border-bottom: 1px solid #f1f5f9; display: flex; justify-content: space-between; align-items: center;
  }
  .header-text h3 { margin: 0 0 4px 0; font-size: 16px; color: #1e293b; font-weight: 600; }
  .header-text p { margin: 0; font-size: 13px; color: #64748b; }
  .import-body { padding: 24px; }

  .file-drop {
    display: block; width: 100%; border: 2px dashed #e2e8f0; border-radius: 12px;
    padding: 32px; text-align: center; cursor: pointer; transition: all 0.2s;
    background: #f8fafc;
  }
  .file-drop:hover { border-color: #10b981; background: #ecfdf5; }
  .drop-content { display: flex; flex-direction: column; align-items: center; }
  .file-name { font-weight: 600; color: #059669; }
  .placeholder { color: #64748b; font-size: 13px; }

  .action-footer { display: flex; justify-content: flex-end; align-items: center; gap: 16px; margin-top: 8px; }
  .result-badge { font-size: 13px; color: #475569; display: flex; align-items: center; gap: 8px; }
