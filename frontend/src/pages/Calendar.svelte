<script>
  ...
  import AgendaModal from "./AgendaModal.svelte";
  ...
  let agendaModalData = $state(null); // { date, agenda }
  ...
  function buildCalendarDays(date) {
    ...
    for (let i = startDay - 1; i >= 0; i--) {
      days.push({
        date: new Date(date.getFullYear(), date.getMonth() - 1, prevDays - i),
        outside: true,
      });
    }
  ...
  function buildAgendasMap(list) {
    const map = {};
    if (!list) return map;
    for (const agenda of list) {
      if (agenda.date) {
        const normalizedDate = normalizeDateString(agenda.date);
        if (normalizedDate) {
          if (!map[normalizedDate]) map[normalizedDate] = [];
          map[normalizedDate].push(agenda);
        }
      }
    }
    return map;
  ...
  const agendasByDate = $derived(buildAgendasMap(agendas));
  ...
  async function openAttendanceModal(date) {
    ...
    try {
      ...
      const stats = {
        ...
        permission: dailyAttendances.filter((a) => a.status === "permission")
          .length,
  ...
  function handleKeydown(e) {
    if (e.key === "Escape") {
      ...
      if (showAgendaModal) closeAgendaModal();
    }
  }

  // --- Agenda Handlers ---
  function openAgendaModal(agenda = null, date = null) {
    agendaModalData = {
  ...
  function closeAgendaModal() {
    ...
    agendaModalData = null;
  }

  async function handleAgendaSave(event) {
    const data = event.detail;
    try {
      if (data.deleted) {
        // Agenda deleted in modal, just refresh
      } else if (data.id) {
        await api.updateAgenda(data.id, data);
      } else {
  ...
  function handleDateDblClick(date) {
    // Only allow if user can manage or if it's their own calendar (everyone can add personal agenda)
    // Requirement says "Allow allowing users to add agenda entries"
    openAgendaModal(null, date);
  }
...
<div class="w-full max-w-7xl mx-auto space-y-6">
  <div
    class="card p-0 overflow-hidden bg-white border-slate-200 shadow-sm rounded-xl"
  >
    <div
      class="p-4 sm:p-6 border-b border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4 bg-slate-50/50"
    >
      <div class="flex items-center justify-between w-full md:w-auto gap-4">
        <button
          onclick={goPrevMonth}
          class="w-8 h-8 flex items-center justify-center rounded-full bg-white border border-slate-200 text-slate-500 hover:bg-slate-100 hover:text-slate-700 transition-colors shadow-sm"
        ...
        <h3
          class="text-lg sm:text-xl font-bold text-slate-800 text-center min-w-[150px]"
        >
      ...
      <div class="flex flex-wrap md:flex-nowrap w-full md:w-auto gap-2">
        <button
          ...
          class="flex-1 md:flex-none px-5 py-2 rounded-full text-sm font-semibold transition-all flex items-center justify-center gap-2 {viewMode ===
          'attendance'
        ...
        >
          ...
          {:else}
            <svg
              ...
              stroke="currentColor"
              stroke-width="2"
              ><path d="M15 12h-5" /><path d="M15 8h-5" /><path
                d="M19 17V5a2 2 0 0 0-2-2H4"
              /><path
                d="M8 21h12a2 2 0 0 0 2-2v-1a1 1 0 0 0-1-1H11a1 1 0 0 0-1 1v2a2 2 0 0 0 2 2z"
              /></svg
            >
            Tinjau Agenda
          {/if}
    ...
    {#if loading}
      <div class="p-8 text-center text-muted" style="min-height: 750px;">
      ...
      <div class="p-2 sm:p-6">
        <div class="grid grid-cols-7 gap-1 sm:gap-2 mb-2">
          {#each ["MIN", "SEN", "SEL", "RAB", "KAM", "JUM", "SAB"] as dayName}
            <div
              class="text-center py-2 text-xs font-bold text-slate-400 uppercase tracking-wider"
            >
              {dayName}
            </div>
          {/each}
        ...
        <div
          class="grid grid-cols-7 gap-1 sm:gap-2 auto-rows-fr"
          style="height: 672px;"
        >
              ...
              class="relative p-2 border rounded-xl transition-all flex flex-col gap-1 text-left
                ...
                : isRedDate
                  ...
                  : 'bg-white border-slate-100'}
                     {isSelected(day.date)
                ? '!ring-2 !ring-indigo-600 !border-transparent z-10'
                : ''}
                     hover:border-indigo-300 hover:shadow-sm group"
              style={day.outside ? "opacity: 0.5;" : ""}
              onclick={() => selectDate(day.date)}
              ondblclick={() => handleDateDblClick(day.date)}
              title={holidayInfo ? holidayInfo.name : ""}
            >
              <div class="flex justify-between items-start w-full">
                <span
                  class="text-sm font-semibold {isToday(day.date)
                    ? 'text-indigo-600 bg-indigo-100 w-6 h-6 rounded-lg flex items-center justify-center'
                    : isRedDate
                      ? 'text-rose-500'
                      : 'text-slate-700'}"
                >
                ...
                {#if holidayInfo}
                  <span
                    class="hidden sm:inline-block text-[9px] bg-rose-100 text-rose-600 px-1.5 py-0.5 rounded font-medium leading-tight text-right max-w-[85px] break-words"
                  >
                    {holidayInfo.name}
                  </span>
                {/if}
              ...
              <div
                class="flex-1 w-full flex flex-col gap-1 overflow-hidden mt-1"
              >
                <!-- Agendas (Always visible atop) -->
                {#if dayAgendas.length > 0}
                  {#each dayAgendas as agenda}
                    <div
                      role="button"
                      tabindex="0"
                      class="hidden sm:block w-full text-left px-1.5 py-0.5 rounded border border-blue-200 bg-blue-50 text-blue-700 text-xs font-bold truncate hover:bg-blue-100 cursor-pointer shrink-0 z-10 relative"
                      onclick={(e) => {
                        e.stopPropagation();
                        openAgendaModal(agenda, day.date);
                      }}
                      onkeydown={(e) => {
                    ...
                    >
                      {agenda.time ? agenda.time.substring(0, 5) : ""}
                      {agenda.title}
                    </div>
                  {/each}
                  <!-- Mobile Dot -->
                  <div
                    class="sm:hidden w-2 h-2 rounded-full bg-blue-500 mx-auto mb-0.5"
                  ></div>
                {/if}

                <!-- Hover Add Button (Desktop) -->
                <div
                  class="absolute bottom-1 right-1 z-20 hidden sm:flex opacity-0 group-hover:opacity-100 transition-all duration-200"
                >
                  <div
                    role="button"
                    tabindex="0"
                    class="w-6 h-6 rounded-full bg-indigo-600 text-white flex items-center justify-center shadow-md hover:bg-indigo-700 hover:scale-110 transition-transform cursor-pointer"
                    onclick={(e) => {
                      e.stopPropagation();
                      openAgendaModal(null, day.date);
                    }}
                    onkeydown={(e) => {
                      if (e.key === "Enter" || e.key === " ") {
                        e.stopPropagation();
                ...
                {#if viewMode === "attendance"}
                    ...
                    {#if dayEvents}
                      ...
                      <div class="flex flex-wrap gap-1 content-start mt-1">
                        {#each dayEvents as event}
                          ...
                          {@const bgClass =
                            statusClasses[event.status] || "bg-slate-500"}
                  ...
                  {:else}
                    ...
                    {#if att}
                      ...
                      <div
                        class="hidden sm:flex items-center gap-1.5 px-2 py-1 rounded-md text-[10px] sm:text-xs font-semibold {attColor.bg} {attColor.text}"
                      >
                        <span class="truncate">
                          {att.check_in_time
                      ...
                      <div
                        class="sm:hidden w-2 h-2 rounded-full mx-auto {getAttendanceDotColor(
                          att.status,
                        )}"
                      ></div>
                    {/if}
                  ...
                  {#if dayTasks.length > 0}
                    <div class="flex-1 min-h-0 overflow-hidden space-y-0.5">
                      {#each dayTasks as task}
                        ...
                        >
                          <div
                            class="px-1.5 py-0.5 rounded border text-[10px] font-medium truncate w-full {getPriorityColor(
                              task.priority,
                            )} hover:ring-1 hover:ring-offset-1 hover:ring-indigo-400"
                          >
                            {task.title}
                          </div>
                        </div>
                    ...
                    <div
                      class="sm:hidden w-2 h-2 rounded-full {getPriorityDotColor(
                        highestPriority,
                      )} mx-auto"
                    ></div>
                  {/if}
        ...
        <!-- Legend -->
        <div
          class="flex flex-wrap items-center gap-4 mt-6 pt-4 border-t border-slate-100 text-xs"
        >
          <!-- Holiday Legend (always visible) -->
          <div class="flex items-center gap-2">
            <span class="w-3 h-3 rounded bg-rose-50 border border-rose-200"
            ></span>
          ...
          {#if viewMode === "attendance"}
            ...
            {#if canManage}
              <div
                class="ml-auto text-slate-400 hidden sm:flex items-center gap-1"
              >
                <svg
                  width="14"
                  height="14"
  ...
  <div class="min-h-40">
    {#if monthHolidays.length > 0}
      <div
        class="card p-4 bg-gradient-to-br from-rose-50 to-rose-100/30 border-rose-200 rounded-xl"
      >
        <h4
          class="text-sm font-bold text-rose-700 uppercase tracking-wider mb-3 flex items-center gap-2"
        >
          <svg
            width="16"
            height="16"
        ...
        <div class="space-y-2">
          {#each monthHolidays as holiday}
            ...
            {#if normalizedDate}
              <div
                class="flex items-center gap-2 p-2 bg-white/60 rounded-lg border border-rose-100"
              >
                <span
                  class="w-8 h-8 bg-rose-100 text-rose-600 rounded-lg flex items-center justify-center font-bold text-sm"
                >
  ...
  {#if viewMode === "attendance"}
    {#if showModal}
      {@const dateKey = toDateKey(parseDate(modalData?.date) || selectedDate)}
      <!-- Note: modalData.date is formatted string, so we might need selectedDate or parse it back. 
           Actually openAttendanceModal(date) sets selectedDate before calling logic? 
           No, it separates. But we can derive from selectedDate since modal is for that date. -->
      {@const dayAgendas = agendasByDate[toDateKey(selectedDate)] || []}
      ...
      >
        ...
        <!-- svelte-ignore a11y_no_static_element_interactions -->
        <div
          class="relative bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden"
        ...
        >
          ...
          >
            <div>
              <h3
                class="text-lg cursor-pointer font-bold text-slate-800 flex items-center gap-2"
              >
                <svg
                  width="20"
                  height="20"
          ...
          <div class="p-6 overflow-y-auto">
            <div class="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
              <div
                class="bg-gradient-to-br from-emerald-50 to-emerald-100/50 p-4 rounded-xl text-center border border-emerald-100"
              >
                <div class="text-3xl font-black text-emerald-600 mb-1">
                  {modalData?.present || 0}
                </div>
                <div
                  class="text-xs font-bold text-emerald-600/70 uppercase tracking-wider"
                >
                  Hadir
                </div>
              </div>
              <div
                class="bg-gradient-to-br from-amber-50 to-amber-100/50 p-4 rounded-xl text-center border border-amber-100"
              >
                <div class="text-3xl font-black text-amber-600 mb-1">
                  {modalData?.late || 0}
                </div>
                <div
                  class="text-xs font-bold text-amber-600/70 uppercase tracking-wider"
                >
                  Terlambat
                </div>
              </div>
              <div
                class="bg-gradient-to-br from-rose-50 to-rose-100/50 p-4 rounded-xl text-center border border-rose-100"
              >
                <div class="text-3xl font-black text-rose-600 mb-1">
                  {modalData?.absent || 0}
                </div>
                <div
                  class="text-xs font-bold text-rose-600/70 uppercase tracking-wider"
                >
                  Belum Absen
                </div>
              </div>
              <div
                class="bg-gradient-to-br from-sky-50 to-sky-100/50 p-4 rounded-xl text-center border border-sky-100"
              >
                <div class="text-3xl font-black text-sky-600 mb-1">
                  {modalData?.permission || 0}
                </div>
                <div
                  class="text-xs font-bold text-sky-600/70 uppercase tracking-wider"
                >
                  Izin
                </div>
              </div>
              <div
                class="bg-gradient-to-br from-purple-50 to-purple-100/50 p-4 rounded-xl text-center border border-purple-100"
              >
                <div class="text-3xl font-black text-purple-600 mb-1">
                  {modalData?.sick || 0}
                </div>
                <div
                  class="text-xs font-bold text-purple-600/70 uppercase tracking-wider"
                >
                  Sakit
                </div>
              </div>
              <div
                class="bg-slate-50 p-4 rounded-xl text-center border border-slate-100"
              >
                <div class="text-3xl font-black text-slate-600 mb-1">
                  {modalData?.total || 0}
                </div>
                <div
            ...
            {#if dayAgendas.length > 0}
              <div class="mb-6 mt-6 border-t border-slate-100 pt-6">
                <h4
                ...
                <div class="space-y-2">
                  {#each dayAgendas as agenda}
                    <button
                      class="w-full text-left p-3 rounded-xl border border-blue-200 bg-blue-50 hover:bg-blue-100 transition-colors group relative"
                      onclick={() => openAgendaModal(agenda, selectedDate)}
                    >
                      <div class="flex justify-between items-start mb-1">
                        <div class="flex items-center gap-2">
                          <span
                            class="font-mono font-bold text-blue-600 bg-white px-2 py-0.5 rounded text-xs border border-blue-100 shadow-sm"
                          >
                            {agenda.time
                      ...
                      {#if agenda.description}
                        <p
                          class="text-xs text-slate-600 ml-[calc(2rem+.5rem)] line-clamp-2"
                        >
                          {agenda.description}
                        </p>
                      {/if}
            ...
            {#if modalData?.attendances && modalData.attendances.length > 0}
              ...
              <div>
                ...
                >
                  ...
                  {#if attendanceSearchQuery}
                    <span class="text-xs font-normal text-slate-400"
                      >({filteredAttendances.length} hasil)</span
                    >
                  {/if}
                </h4>
                ...
                {#if filteredAttendances.length > 0}
                  ...
                  >
                    <table class="w-full text-left text-sm">
                      <thead
                        class="bg-slate-50 text-slate-500 font-semibold uppercase text-xs"
                      >
                        <tr>
                      ...
                      <tbody class="divide-y divide-slate-100">
                        {#each filteredAttendances as att}
                          ...
                          <tr class="hover:bg-slate-50/50">
                            ...
                            <td class="px-4 py-3">
                              <span
                                class="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold {attColor.bg} {attColor.text}"
                              >
                                {getAttendanceLabel(att.status)}
                              </span>
                            </td>
                            <td
                              class="px-4 py-3 text-center font-mono text-xs text-slate-500"
                            >
                              {att.check_in_time
                                ? new Date(
                                    att.check_in_time,
                ...
                {:else}
                  <div
                    class="text-center py-8 text-slate-400 bg-slate-50 rounded-xl"
                  >
                    <p>
                      Tidak ditemukan siswa dengan nama "{attendanceSearchQuery}"
                    </p>
                  </div>
                {/if}
              </div>
    ...
    >
      ...
      >
        ...
        <div class="p-6 overflow-y-auto">
          ...
          {#if holiday}
            <div
              class="mb-4 p-3 bg-rose-50 border border-rose-100 rounded-xl flex items-center gap-3"
            >
              <span
                class="w-8 h-8 bg-rose-100 text-rose-600 rounded-lg flex items-center justify-center font-bold text-sm"
              >
          ...
          {#if dailyTasks.length > 0}
                ...
                {#each filteredTasks as task}
                  ...
                  >
                    <div class="flex justify-between items-start mb-2">
                      <h5
                        class="font-bold text-slate-800 group-hover:text-indigo-600 transition-colors line-clamp-1"
                      >
                        {task.title}
                      </h5>
                      <span
                        class="px-2 py-0.5 rounded text-[10px] font-bold border uppercase {getPriorityColor(
                          task.priority,
                        )}"
                      >
                        {task.priority}
            ...
            {:else}
              <div
                class="text-center py-12 text-slate-400 bg-slate-50 rounded-xl border border-dashed border-slate-200"
              >
                <p>
                  Tidak ditemukan agenda dengan kata kunci "{taskSearchQuery}"
                </p>
              </div>
            {/if}
          {:else}
  ...
  <AgendaModal
    ...
    date={agendaModalData?.date}
    agenda={agendaModalData?.agenda}
    on:close={closeAgendaModal}
    on:save={handleAgendaSave}
  />
</div>
