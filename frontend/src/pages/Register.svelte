<script>
  ...
  onMount(() => {
    ...
    if (oauthStatus === 'google_unregistered' || params.get('status') === 'unregistered') {
  ...
  async function handleRegister(e) {
    ...
    // Backend only exposes public registration for interns.
    if (role !== 'intern') {
      error = 'Pendaftaran pembimbing harus melalui admin. Pilih Peserta Magang atau hubungi admin.';
      loading = false;
      return;
    }
    ...
    try {
      ...
      console.error(err);
      error = err.response?.data?.message || err.message || 'Pendaftaran gagal. Silakan coba lagi.';
      loading = false;
    }
  }
    ...
    {#if isSubmitted}
      ...
      <div class="card p-0 overflow-hidden text-center fade-in">
         <div class="p-6 md:p-8">
            <div class="w-16 h-16 mx-auto mb-4 rounded-2xl flex items-center justify-center text-2xl bg-green-100 text-green-600">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    <path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path>
                    <polyline points="22 4 12 14.01 9 11.01"></polyline>
                </svg>
            </div>
            <h1 class="text-xl font-bold text-slate-800 mb-2">Pendaftaran Terkirim</h1>
            <p class="text-slate-500 text-sm">Akun Anda akan segera kami proses.</p>
         </div>
         ...
         <div class="p-6 md:p-8 bg-slate-50 border-t border-slate-200">
            <div class="p-4 rounded-xl bg-yellow-50 border border-yellow-200">
                <div class="flex gap-3">
                    <svg class="text-yellow-500 mt-0.5 flex-shrink-0" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="8" x2="12" y2="12"></line><line x1="12" y1="16" x2="12.01" y2="16"></line></svg>
                    <p class="text-sm text-yellow-800">
                        Setelah mendaftar, akun Anda akan menunggu persetujuan dari admin. Anda akan menerima notifikasi setelah akun diaktifkan.
                    </p>
                </div>
            </div>
             <button class="btn btn-secondary w-full mt-6" onclick={() => replace('/login')}>
                Kembali ke Login
            </button>
         </div>
      </div>
    {:else}
      ...
      <form class="card p-0 overflow-hidden" onsubmit={handleRegister}>
        ...
        <div class="form-section text-center bg-gray-50">
            <div class="w-16 h-16 mx-auto mb-4 rounded-2xl flex items-center justify-center text-2xl text-white bg-gradient-to-br from-purple-500 to-indigo-500 shadow-lg shadow-indigo-500/30">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><line x1="22" x2="18" y1="8" y2="8"/><line x1="20" x2="20" y1="6" y2="10"/></svg>
            </div>
            <h1 class="text-xl font-extrabold text-slate-800 mb-1 tracking-tight">Pendaftaran</h1>
            <p class="text-slate-500 text-sm">Pilih jenis akun dan isi data lengkap untuk mendaftar</p>
        </div>
        ...
        <div class="form-section">
            <div class="section-header">
                ...
                <div>
                    ...
                    <p class="text-sm text-slate-500">Pilih jenis akun yang ingin Anda buat</p>
                </div>
            </div>

            <div class="role-selector">
                <button type="button" class="role-option" class:active={role === 'intern'} onclick={() => role = 'intern'}>
                ...
                <button type="button" class="role-option" class:active={role === 'supervisor'} onclick={() => role = 'supervisor'}>
                     <div class="icon-wrapper supervisor" class:active={role === 'supervisor'}>
        ...
        <div class="form-section intern-fields fade-in">
             <div class="section-header">
                ...
                <div>
                    ...
                    <p class="text-sm text-slate-500">Informasi tentang diri Anda</p>
                </div>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="form-group mb-0">
             ...
             <div class="section-header mt-6">
                <div class="section-icon bg-green-100 text-green-600">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" x2="16" y1="2" y2="6"/><line x1="8" x2="8" y1="2" y2="6"/><line x1="3" x2="21" y1="10" y2="10"/></svg>
                </div>
                <div>
        ...
        <div class="form-section supervisor-fields fade-in">
             <div class="section-header">
                ...
                <div>
                    ...
                    <p class="text-sm text-slate-500">Informasi tentang diri Anda sebagai pembimbing</p>
                </div>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="form-group mb-0">
        ...
        <div class="form-section pt-0">
             <div class="p-4 rounded-xl mb-5 bg-purple-50 border border-purple-200/80">
                <div class="flex gap-3">
                    <svg class="text-purple-500 mt-0.5 flex-shrink-0" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <button type="submit" class="btn btn-primary w-full py-3 text-sm" disabled={loading}>
                 {#if loading}
            ...
            <p class="text-center text-sm text-slate-500 mt-5">
                Sudah punya akun?
                <a href="/login" use:route class="text-purple-600 hover:text-purple-700 font-medium">
                    Masuk di sini
                </a>
            </p>
        </div>
...
<style>
  ...
  .icon-wrapper.intern { background-color: #dcfce7; color: #16a34a; }
  .icon-wrapper.supervisor { background-color: #dbeafe; color: #2563eb; }
  .icon-wrapper.active.intern { background: linear-gradient(135deg, #22c55e 0%, #15803d 100%); color: white; }
  .icon-wrapper.active.supervisor { background: linear-gradient(135deg, #3b82f6 0%, #1d4ed8 100%); color: white; }
