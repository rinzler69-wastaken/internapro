    ...
    <div class="card-header-row border-b">
      <div class="flex flex-wrap md:flex-nowrap w-full md:w-auto gap-2">
        ...
        {#if auth.user?.role !== "intern"}
          <button
            class="cursor-pointer flex-none md:flex-none w-10 h-10 md:w-auto md:h-auto md:px-5 md:py-2 rounded-full text-sm font-semibold bg-white text-slate-900 border border-slate-200 hover:border-slate-300 transition-all flex items-center justify-center gap-2"
            onclick={exportTasks}
            disabled={exporting}
          >
            {#if exporting}
              <div
                class="w-4 h-4 border-2 border-slate-200 border-t-slate-900 rounded-full animate-spin"
              ></div>
            {:else}
              <svg
                xmlns="http://www.w3.org/2000/svg"
            ...
            <span class="hidden md:inline"
              >{exporting ? "Menyiapkan..." : "Export"}</span
            >
          </button>
        {/if}
        <button
