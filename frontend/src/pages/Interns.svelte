<script>
  ...
  const isSupervisor = $derived(auth.user?.role === 'supervisor' || auth.user?.role === 'pembimbing');
  ...
  async function createIntern() {
    ...
    const supervisorId = isSupervisor ? auth.user?.id : (form.supervisor_id ? parseInt(form.supervisor_id, 10) : null);
  ...
  async function updateIntern() {
    ...
    // Convert supervisor_id to number or null
    const supervisorId = isSupervisor ? auth.user?.id : (editForm.supervisor_id ? parseInt(editForm.supervisor_id, 10) : null);
...
<div class="page-container animate-fade-in">
  ...
  <div class="card table-card animate-slide-up" style="animation-delay: 0.1s;">
    <div class="card-header-row border-b">
          ...
          <button class="flex-1 md:flex-none px-5 py-2 rounded-full text-sm font-semibold bg-white text-slate-900 border border-slate-200 hover:border-slate-300 transition-all flex items-center justify-center gap-2" onclick={fetchInterns}>
    ...
    <div class="toolbar">
      <div class="search-wrapper">
        ...
        <input 
          ...
          onkeydown={(e) => e.key === 'Enter' && (clearTimeout(searchTimeout), fetchInterns())}
          placeholder="Cari Nama, Email, atau Sekolah..." 
          class="search-input"
        />
      </div>
      
      <select bind:value={filterStatus} onchange={fetchInterns} class="filter-select">
...
<div class="user-info">
  ...
  <div class="user-details">
    <span class="user-name">{i.full_name || '-'}</span>
                  ...
                  <td class="text-muted">{i.school || i.institution_name || '-'}</td>
                  ...
                  <td class="text-right">
                    <div class="action-buttons responsive">
                      ...
                      {:else}
                        <button class="px-2 py-2 rounded-full text-sm font-semibold border border-slate-200 hover:border-slate-300 text-slate-200 hover:text-white bg-slate-900 hover:bg-slate-800 pointer-cursor transition-all flex items-center justify-center gap-2 flex-1" onclick={() => startEdit(i)} title="Edit data">
                        ...
                        <button class="px-2 py-2 rounded-full text-sm font-semibold border border-red-200 hover:border-red-300 text-slate-200 hover:text-white bg-red-600 hover:bg-red-500 pointer-cursor transition-all flex items-center justify-center gap-2 flex-1" onclick={() => handleDeny(i.id, i.full_name)} title="Hapus">
          ...
          <div class="entry-card">
            ...
            <div class="entry-head" onclick={() => toggleExpand(i.id)}>
              ...
              <button class="expand-btn">
                <span class="material-symbols-outlined transition-transform duration-200 {expandedInterns[i.id] ? 'rotate-180' : ''}">expand_more</span>
              </button>
            </div>
            ...
            <div class="entry-details" transition:slide={{ duration: 200 }}>
              <div class="detail-row">
                ...
                <span class={`status-badge equal-badge ${i.status === 'active' ? 'bg-emerald-100 text-emerald-700' : i.status === 'pending' ? 'bg-yellow-100 text-yellow-700' : 'bg-slate-100 text-slate-600'}`}>
                  {i.status || 'inactive'}
                </span>
              </div>
              <div class="detail-row">
                <div class="detail-label">SEKOLAH</div>
                <div class="detail-value">{i.school || i.institution_name || '-'}</div>
              </div>
              <div class="detail-row">
              ...
              <div class="detail-row">
                ...
                <div class="detail-value">{i.start_date ? i.start_date.slice(0,10) : '-'}</div>
              </div>
              <div class="detail-row">
                <div class="detail-label">SELESAI</div>
                <div class="detail-value">{i.end_date ? i.end_date.slice(0,10) : '-'}</div>
              </div>
              <div class="detail-row">
              ...
              <div class="mobile-actions mt-4 pt-4 border-t border-slate-100">
                {#if i.status === 'pending'}
                <button class="mini-btn mobile danger" onclick={(e) => { e.stopPropagation(); handleDeny(i.id, i.full_name); }}>
                  <span class="material-symbols-outlined">close</span>
                  <span class="btn-text">Tolak</span>
                </button>
                <button class="mini-btn mobile success" onclick={(e) => { e.stopPropagation(); handleApprove(i.id, i.full_name); }}>
                  <span class="material-symbols-outlined">check</span>
                  <span class="btn-text">Terima</span>
                </button>
                {:else}
                <button class="mini-btn mobile" onclick={(e) => { e.stopPropagation(); startEdit(i); }}>
    ...
    <!-- svelte-ignore a11y_no_static_element_interactions -->
      <div class="fixed inset-0 z-120 flex items-center justify-center p-4 sm:p-6" use:portal>
        ...
        <div class="p-6 overflow-y-auto">
            <div class="form-grid">
              <div class="form-group">
                <label class="form-label" for="full_name">Nama Lengkap <span class="text-red-500">*</span></label>
                <input class="input" bind:value={form.full_name} id="full_name" placeholder="Contoh: Budi Santoso" required />
              </div>
              <div class="form-group">
                <label class="form-label" for="email">Email <span class="text-red-500">*</span></label>
                <input class="input" type="email" bind:value={form.email} id="email" placeholder="email@sekolah.com" required />
              </div>
              <div class="form-group">
              ...
              <div class="form-group">
                <label class="form-label" for="school">Sekolah <span class="text-red-500">*</span></label>
                <input class="input" bind:value={form.school} id="school" placeholder="Nama Sekolah" required />
              </div>
              <div class="form-group">
              ...
              <div class="form-group">
                ...
                <input class="input" bind:value={form.address} id="address" placeholder="Alamat domisili" />
              </div>
              {#if isAdmin}
                <div class="form-group">
                  <label class="form-label" for="supervisor">Pembimbing</label>
                  <select class="input" id="supervisor" bind:value={form.supervisor_id}>
                    <option value="">Tidak ada</option>
                    {#each supervisors as s}
                      <option value={s.user_id}>{s.full_name || s.name}{s.institution ? ` - ${s.institution}` : ''}</option>
                    {/each}
                  </select>
                </div>
              {/if}
        ...
        <div class="p-5 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-3">
            ...
            <button class="btn btn-primary" onclick={createIntern}>Simpan Data</button>
        </div>
      </div>
    </div>
  {/if}
    ...
    <!-- svelte-ignore a11y_no_static_element_interactions -->
      <div class="fixed inset-0 z-120 flex items-center justify-center p-4 sm:p-6" use:portal>
        ...
        <div class="p-6 overflow-y-auto">
            <div class="form-grid">
              <div class="form-group">
                ...
                <input class="input" bind:value={editForm.full_name} id="edit_full_name" placeholder="Contoh: Budi Santoso" />
              </div>
              <div class="form-group">
                <label class="form-label" for="edit_email">Email</label>
                <input class="input" type="email" bind:value={editForm.email} id="edit_email" placeholder="email@sekolah.com" disabled />
              ...
              <div class="form-group">
                ...
                <input class="input" bind:value={editForm.school} id="edit_school" placeholder="Nama Sekolah" />
              </div>
              <div class="form-group">
                <label class="form-label" for="edit_department">Jurusan</label>
                <input class="input" bind:value={editForm.department} id="edit_department" placeholder="Jurusan" />
              </div>
              <div class="form-group">
                <label class="form-label" for="edit_address">Alamat</label>
                <input class="input" bind:value={editForm.address} id="edit_address" placeholder="Alamat domisili" />
              </div>
              {#if isAdmin}
                <div class="form-group">
                  <label class="form-label" for="edit_supervisor">Pembimbing</label>
                  <select class="input" id="edit_supervisor" bind:value={editForm.supervisor_id}>
                    <option value="">Tidak ada</option>
                    {#each supervisors as s}
                      <option value={s.user_id}>{s.full_name || s.name}{s.institution ? ` - ${s.institution}` : ''}</option>
                    {/each}
              ...
              <div class="form-group">
                ...
                <input class="input" type="date" bind:value={editForm.start_date} id="edit_start_date" />
              </div>
              <div class="form-group">
                <label class="form-label" for="edit_end_date">Selesai</label>
                <input class="input" type="date" bind:value={editForm.end_date} id="edit_end_date" />
              </div>
            </div>
        </div>
        ...
        <div class="p-5 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-3">
            ...
            <button class="btn btn-primary" onclick={updateIntern}>Simpan Perubahan</button>
        </div>
      </div>
    </div>
  {/if}
...
.pagination-pill {
  ...
  .status-inactive, .status-cancelled, .status-completed {
    background-color: #f3f4f6;
    color: #4b5563;
    border: 1px solid #e5e7eb;
  }
