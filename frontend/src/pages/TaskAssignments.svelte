<script>
  ...
  async function fetchAssignments() {
    loading = true;
    try {
      const res = await api.getTaskAssignments({
  ...
  <div class="container animate-fade-in">
    ...
    <div class="header">
      <div>
        ...
        <p class="subtitle">
          Kelola dan pantau progres tugas yang diberikan kepada peserta.
        </p>
      </div>
          ...
          <table class="table">
            ...
            <tbody>
              {#each assignments as item}
                <tr
                  onclick={() => goto(`/task-assignments/${item.id}`)}
                ...
                >
                  ...
                  <td>
                    <span
                      class={`priority-badge ${getPriorityColor(item.priority)}`}
                    >
                  ...
                  <td style="min-width: 160px;">
                    <div class="progress-wrapper">
                      <div class="progress-bar-bg">
                        <div
                          class="progress-bar-fill {getProgressColor(
                            item.stats?.progress_percentage || 0,
                          )}"
                          style="width: {item.stats?.progress_percentage || 0}%"
                        ></div>
                      </div>
                      <span class="progress-text"
                        >{item.stats?.progress_percentage || 0}%</span
                      >
                    </div>
                  </td>
                  <td class="text-right">
                    <button
                      class="btn-icon"
                      aria-label={`Lihat rincian ${item.intern_name || "penugasan"}`}
                    >
                      <svg
                        width="20"
                        height="20"
