<script>
  ...
  async function load() {
    ...
    try {
      // Fetch specific intern
      const detail = await api.getIntern(internId);
      ...
      console.error(err);
      error = err.message || "Gagal memuat detail intern";
    } finally {
      loading = false;
      await tick();
  ...
  async function loadInternMetrics() {
    ...
    try {
      const results = await Promise.allSettled([
        api.get("/dashboard/intern", { intern_id: internId }),
        api.getTasks({ intern_id: internId, limit: 400 }),
        api.getAttendance({ intern_id: internId, limit: 400 }),
        api.getAssessments({ intern_id: internId, limit: 30 }),
      ]);
  ...
  async function handleDownloadReport() {
    try {
      const { blob, filename } = await api.downloadInternReport(internId);
  ...
  function handleQuickAssessment() {
    ...
    {:else}
      <div class="card hero">
        ...
        <div class="hero-right">
          ...
          <button
            class="btn-report cursor-pointer"
      ...
      {#if intern}
        ...
        <div class="card quick-actions-card">
          ...
          <div class="quick-actions-grid">
            <button class="action-btn task" onclick={handleQuickTask}>
              <div class="icon-wrap">
                <span class="material-symbols-outlined">add_task</span>
              </div>
              <div class="action-info">
                <span class="action-title">Tambah Tugas</span>
                <span class="action-desc"
                  >Berikan tugas baru untuk {intern.full_name}</span
                >
              </div>
            </button>
            <button class="action-btn assess" onclick={handleQuickAssessment}>
              <div class="icon-wrap">
                <span class="material-symbols-outlined">grading</span>
              </div>
              <div class="action-info">
                <span class="action-title">Beri Penilaian</span>
                <span class="action-desc"
                  >Evaluasi kinerja {intern.full_name}</span
                >
              </div>
            </button>
          </div>
...
<TaskCreateModal
  ...
  onSuccess={load}
  preSelected={intern ? [{ id: intern.user_id, label: intern.full_name }] : []}
/>

<AssessmentCreateModal
  isOpen={showAssessmentModal}
  onClose={() => (showAssessmentModal = false)}
  onSuccess={load}
  preSelectedInternId={intern?.id}
/>
...
<style>
  ...
  .action-btn.task span.material-symbols-outlined {
    color: #4f46e5;
  }
  .action-btn.assess span.material-symbols-outlined {
    color: #059669;
  }
  .action-info {
    display: flex;
